...
void myproject(
    ...
    if (!loaded_weights) {
        nnet::load_weights_from_txt<_0_scale_t, 8>(s2, "s2.txt");
        nnet::load_weights_from_txt<_0_bias_t, 8>(b2, "b2.txt");
        loaded_weights = true;    }
#endif
    // ****************************************
    ...
    nnet::transpose<input_t, layer3_t, config3>(input_1, layer3_out); // transpose_input_for_input_1

    layer4_t layer4_out[OUT_HEIGHT_4*OUT_WIDTH_4];
    #pragma HLS ARRAY_PARTITION variable=layer4_out complete dim=0
    nnet::transpose<layer3_t, layer4_t, config4>(layer3_out, layer4_out); // pre_transpose_for__0
